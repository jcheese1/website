...
function SheetContent({
  ...
  return (
    <SheetPortal container={container} className="z-50">
      {withOverlay && <SheetOverlay />}
      <BaseSheet.Viewport className="absolute inset-0 flex items-center justify-center overflow-hidden px-4 py-48 sm:px-0">
        <BaseSheet.Popup
          data-slot="sheet-content"
          className={cn(
            "flex max-h-full min-h-0 flex-col rounded-lg bg-popover text-popover-foreground shadow-lg outline-hidden transition ease-in-out data-ending-style:opacity-0 data-starting-style:opacity-0 data-closed:duration-200 data-open:duration-500",
            side === "right" &&
